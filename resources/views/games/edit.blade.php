<x-app-layout>
    <x-slot name="title">
        Данетки - riddles. Редактирование игры.
    </x-slot>
    <x-slot name="header">
    ...
    <form method="POST" action="{{route('games.update', ['game'=>$game]) }}" class="mt-8 pb-8 space-y-6 w-full">
        @csrf
        @method('put')
        <div class="mx-4">
            ...
            <input type="datetime-local" name="starts_at" class="px-4 w-full rounded-sm border-gray-300"
                placeholder="Начало игры" value="{{ $game->starts_at?->format('Y-m-d\TH:i') }}" required>
        </div>
        @error('starts_at')
            ...
            <div class="sm:col-span-2">
                ...
                <select name="status" class="w-full rounded-sm border-gray-300">
                    @foreach(\App\Enums\GameStatus::cases() as $status)
                        <option value="{{ $status->value }}"  @selected($game->status==$status)>{{ $status->label() }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="mx-2">
            ...
            <input name="host_name" type="text" value="{{$hostName}}" class="w-full rounded-sm border-gray-300 placeholder-gray-800"
